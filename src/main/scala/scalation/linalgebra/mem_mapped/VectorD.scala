...
import scalation.util.{Error, MM_ArrayD}
import scalation.util.MM_SortingD.{iqsort, qsort2}
      ...
      extends Traversable [Double] with PartiallyOrdered [VectorD] with Vec with Error with Serializable
{
     ...
     */
    def this (u: MM_ArrayD) { this (u.length, u) }
    ...
    def expand (more: Int = dim): VectorD =
    {
        ...
        else          new VectorD (dim + more, MM_ArrayD.concat (v, new MM_ArrayD (more)))
    } // expand
     ...
     */
    def setAll (u: MM_ArrayD) { for (i <- range) v(i) = u(i) }
     ...
     */
    def sum: Double = v.foldLeft (0.0)((s: Double, x: Double) => s + x)
   ...
   //:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    ...
    {
        var count = 1
        val us = new VectorD (this); us.sort2 ()               // sorted vector
