...
import scalation.util.MM_SortingR.{iqsort, qsort2}
      ...
      extends Traversable [Real] with PartiallyOrdered [VectorR] with Vec with Error with Serializable
{
     ...
     */
    def this (u: MM_ArrayR) { this (u.length, u) }
    ...
    def expand (more: Int = dim): VectorR =
    {
        ...
        else          new VectorR (dim + more, MM_ArrayR.concat (v, new MM_ArrayR (more)))
    } // expand
     ...
     */
    def setAll (u: MM_ArrayR) { for (i <- range) v(i) = u(i) }
     ...
     *  @param f  the function to apply
     */
    def map (f: Real => Real): VectorR = new VectorR (v.map (f))
     ...
     */
    def sum: Real = v.foldLeft (_0)((s: Real, x: Real) => s + x)
   ...
   //:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    ...
    {
        var count = 1
        val us = new VectorR (this); us.sort2 ()               // sorted vector
 ...
 *  > run-main scalation.linalgebra.mem_mapped.VectorRTest
 */
